...
By Alan, named after Kieran's cat\n`
  },
  ls: function (params) {
    if (params && params.indexOf("l") != -1) {
      return (
        "total " +
        Object.keys(Directories).length * 4 +
        "\n" +
  ...
  cat: function () {
    const templ = document.getElementById("cat").cloneNode(true)
  ...
  sudo: function (params) {
    ...
    switch (pArray[0]) {
      ...
      case "reboot":
        ...
        break
      default:
        if (pArray[0] in Commands) return Commands[pArray[0]]()
...
;(function () {
  ...
  if (!("addEventListener" in b) || !("keys" in Object)) {
    d.getElementsByClassName("content")[0].innerHTML =
  ...
  if ("localStorage" in window && localStorage.theme) {
    var themeRadio = d.querySelector(
      "input[name=theme][value=" + localStorage.theme + "]",
    )
    if (themeRadio) {
  ...
  var tWindow = d.getElementsByClassName("window")[0],
    ...
    setWindowPos = function (x, y) {
      if (x < 0) x = 0
      if (y < 0) y = 0
      if (x > window.innerWidth - tWindow.offsetWidth)
        x = window.innerWidth - tWindow.offsetWidth
      if (y > window.innerHeight - tWindow.offsetHeight)
        y = window.innerHeight - tWindow.offsetHeight
