...
const ThemeMenu = () => {
  ...
  const dropdownClass = `absolute right-0 mt-1 w-40 py-1 z-10 rounded-md shadow-lg bg-white dark:bg-purple-800 border dark:border-purple-600`
                ...
                <li
                  className={
                    `flex items-center appearance-none w-full px-4 py-2 text-sm focus:outline-none ${
                      selected ?
                        `text-white ${active ? 'bg-yellow-600' : 'bg-yellow-500'} bg-pan-30-smooth text-shadow` :
                        `${active ? 'bg-gray-100 dark:bg-purple-700' : ''} text-gray-700 dark:text-purple-200`
                    }`
                  }
                >
