...
const Portfolio = props => {
  return (
    <section {...props}>
        ...
        {items.map(p => (
          ...
          >
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="text-lg lg:text-xl font-display text-zinc-700 dark:text-zinc-200 group flex flex-col gap-4"
            >
              {p.img && (
                <Shiny className="shadow border dark:border-zinc-700">
                  {PortfolioImage(p)}
                </Shiny>
              )}
              <div>
      ...
      <p className="sm:hidden text-xs italic text-zinc-600 dark:text-zinc-400">
