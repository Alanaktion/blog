...
async function init(modelUrl) {
  ...
  try {
    ...
    console.warn("WebGPU execution provider unavailable, falling back to wasm:", err);
    session = await ort.InferenceSession.create(modelUrl, {
...
function sampleNextToken(logits, generatedIds, { temperature, topP, repetitionPenalty }) {
...
async function generate(prompt, options = {}) {
  ...
  self.postMessage({ type: "done", text: tokenizer.decode(ids.slice(promptLen)) });
}
